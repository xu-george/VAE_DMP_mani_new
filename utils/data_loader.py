...
class TorqueLoader(DataLoader):
    ...
    def normalize_data(self, device):
        ...
        self.max, _ = torch.max(self.torque.view(-1, 2), dim=0)
        self.min, _ = torch.min(self.torque.view(-1, 2), dim=0)
        self.torque_norm = (self.torque - self.min).detach() / (self.max - self.min).detach()  
    ...
    def denormalize_data(self, data):
        ...
        if len(self.max.shape) == 1: 
            self.max = self.max.unsqueeze(0).unsqueeze(-1).detach()
            self.min = self.min.unsqueeze(0).unsqueeze(-1).detach()
...
if __name__ == "__main__":
    ...
    traj_loader.load_data_all(data_path)
